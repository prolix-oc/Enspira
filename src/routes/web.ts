...
import { returnAuthObject, updateUserParameter } from '../core/api-helper.js';
import { logger } from '../core/logger.js';
import { fileURLToPath } from 'url';
import { dirname } from 'path';
import { getChatCount } from '../core/database.js';
import {
  loadPreset,
  loadAllPresets,
  hashPassword,
...
// Extend FastifyReply with view method from @fastify/view
 ...
 */
async function loadTextContent(userId: string, fileName: string): Promise<string> {
  try {
    ...
    logger.error('Web', `Error loading ${fileName} for user ${userId}: ${err.message}`);
    return '';
  }
}
...
async function webRoutes(fastify: FastifyInstance, _options: unknown): Promise<void> {
  ...
  fastify.get('/settings', { preHandler: requireAuth }, async (request, reply) => {
    try {
      ...
      const userWithSocials = { ...user };
      if (!userWithSocials.socials) {
        userWithSocials.socials = {};
      }

      // Render settings page
      return (reply as FastifyReplyWithView).view('settings.njk', {
        pageTitle: 'User Settings',
        settingsActive: true,
        user: userWithSocials,
        timeZones,
        success: (request.query as Record<string, string>).success,
        error: (request.query as Record<string, string>).error,
      });
    } catch (error) {
      const err = error as Error;
      logger.error('Web', `Error serving settings page: ${err.message}`);
      return reply.code(500).send('Error loading settings page');
    }
  });

  // Profile settings update endpoint
  fastify.post<{ Body: ProfileSettingsBody }>(
    '/settings/profile',
    ...
    async (request, reply) => {
      try {
        ...
        const err = error as Error;
        logger.error('Web', `Error updating profile settings: ${err.message}`);
  ...
  fastify.post<{ Body: SocialSettingsBody }>(
    '/settings/socials',
    { preHandler: requireAuth },
    async (request, reply) => {
      try {
        ...
        const platforms = ['twitter', 'tiktok', 'youtube', 'instagram', 'twitch', 'kick'] as const;

        for (const platform of platforms) {
          const key = `socials[${platform}]` as keyof SocialSettingsBody;
          socials[platform] = getFieldValue(request.body[key]);
        }
        ...
        const err = error as Error;
        logger.error('Web', `Error updating social media settings: ${err.message}`);
        return reply.code(500).send({
          success: false,
  ...
  fastify.post<{ Body: PasswordSettingsBody }>(
    '/settings/password',
    { preHandler: requireAuth },
    async (request, reply) => {
      try {
        ...
        if (!user.webPasswordHash || !user.webPasswordSalt) {
          ...
          await updateUserParameter(user.user_id, 'webPasswordHash', passwordData.hash);
          await updateUserParameter(user.user_id, 'webPasswordSalt', passwordData.salt);
        ...
        await updateUserParameter(user.user_id, 'webPasswordHash', passwordData.hash);
        await updateUserParameter(user.user_id, 'webPasswordSalt', passwordData.salt);
        await updateUserParameter(user.user_id, 'webPasswordIterations', passwordData.iterations);
        ...
        const err = error as Error;
        logger.error('Web', `Error updating password: ${err.message}`);
        return reply.code(500).send({
          success: false,
  ...
  fastify.get('/dashboard', { preHandler: requireAuth }, async (request, reply) => {
    try {
      ...
      try {
        ...
        const err = error as Error;
        logger.error('Web', `Error fetching chat stats: ${err.message}`);
      }
      ...
      const followerCount = user.current_followers || 0;
      const lastGame = user.current_game?.name || 'None';

      if (user.twitch_tokens?.streamer?.twitch_user_id) {
        try {
          const { fetchStreamInfo } = await import('../integrations/twitch/eventsub.js');
          const streamInfo = await fetchStreamInfo(user.user_id);

          if (streamInfo && streamInfo.success && streamInfo.isLive && streamInfo.data) {
          ...
          const err = error as Error;
          logger.error('Web', `Error fetching stream info: ${err.message}`);
        }
      }
      ...
      return (reply as FastifyReplyWithView).view('dashboard.njk', {
        pageTitle: 'Dashboard',
      ...
      const err = error as Error;
      logger.error('Web', `Error serving dashboard: ${err.message}`);
  ...
  fastify.get('/character', { preHandler: requireAuth }, async (request, reply) => {
    try {
      ...
      return (reply as FastifyReplyWithView).view('character.njk', {
      ...
      const err = error as Error;
      logger.error('Web', `Error serving character editor: ${err.message}`);
  ...
  fastify.get('/world', { preHandler: requireAuth }, async (request, reply) => {
    try {
      ...
      const commandsList = user.commands_list
        ? user.commands_list.map((cmd) => (typeof cmd === 'string' ? cmd : cmd.name)).join('\n')
        : '';
      const auxBots = user.aux_bots
        ? user.aux_bots.map((bot) => (typeof bot === 'string' ? bot : bot.name)).join('\n')
        : '';

      return (reply as FastifyReplyWithView).view('world.njk', {
      ...
      const err = error as Error;
      logger.error('Web', `Error serving world editor: ${err.message}`);
  ...
  fastify.get('/help', { preHandler: requireAuth }, async (request, reply) => {
    try {
      return (reply as FastifyReplyWithView).view('help.njk', {
        pageTitle: 'Help & Documentation',
        helpActive: true,
      });
    } catch (error) {
      const err = error as Error;
      logger.error('Web', `Error serving help page: ${err.message}`);
      return reply.code(500).send(`Error loading help page: ${err.message}`);
    }
  });
  ...
  fastify.get('/gallery', { preHandler: requireAuth }, async (request, reply) => {
    try {
      ...
      return (reply as FastifyReplyWithView).view('gallery.njk', {
      ...
      const err = error as Error;
      logger.error('Web', `Error serving gallery: ${err.message}`);
  ...
  fastify.get<{ Params: CharacterIdParams }>(
    ...
    async (request, reply) => {
      try {
        ...
        return (reply as FastifyReplyWithView).view('character-details.njk', {
        ...
        const err = error as Error;
        logger.error('Web', `Error serving character details: ${err.message}`);
  ...
  fastify.get('/auth/login', async (request, reply) => {
    return (reply as FastifyReplyWithView).view('login.njk', {
      pageTitle: 'Login',
      error: (request.query as Record<string, string>).error || null,
    });
  });
