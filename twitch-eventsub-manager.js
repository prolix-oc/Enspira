...
async function fetchCurrentTwitchSubscriptions(userId) {
  try {
    ...
    if (!appToken) {
      logger.error(
        "Twitch",
        "Failed to get app access token for subscription check"
      );
      return [];
    }
    ...
    if (response.data && response.data.data) {
      ...
      return response.data.data.filter(
        (sub) => sub.transport && sub.transport.callback === callbackUrl
      );
    }
    ...
    logger.error(
      "Twitch",
      `Error fetching current subscriptions: ${error.message}`
    );
    return [];
  }
}
...
async function registerChatMessageSubscription(userId) {
  try {
    ...
    if (!user.twitch_tokens?.streamer?.twitch_user_id) {
      logger.error(
        "Twitch",
        `Missing streamer ID for user ${userId}, can't register chat events`
      );
    ...
    if (!streamerScopes.includes("channel:moderate")) {
      logger.error(
        "Twitch",
        `User ${userId} is missing channel:moderate scope required for chat events`
      );
      return {
        success: false,
        error:
          "Missing required scopes. Please reconnect your Twitch account with chat reading permissions.",
      };
    }
    ...
    if (result.success) {
      logger.log(
        "Twitch",
        `Successfully registered chat subscription for ${userId}`
      );
      return { success: true, created: true };
    } else {
      logger.error(
        "Twitch",
        `Failed to create chat subscription: ${result.error}`
      );
    ...
    logger.error(
      "Twitch",
      `Error in registerChatMessageSubscription: ${error.message}`
    );
    ...
    for (const user of users) {
      try {
        ...
        if (!chatResult.success) {
          logger.error(
            "Twitch",
            `Failed to register chat subscription for ${user.user_id}: ${chatResult.error}`
          );
        }
...
async function createSubscription(userId, subscriptionConfig, broadcasterId) {
  try {
    ...
    const currentTwitchSubscriptions =
      await fetchCurrentTwitchSubscriptions(userId);

    // Check if we already have this subscription on Twitch's side
    const existingSubscription = currentTwitchSubscriptions.find(
      (sub) =>
        sub.type === subscriptionConfig.type &&
        sub.version === subscriptionConfig.version &&
        JSON.stringify(sub.condition) ===
          JSON.stringify(subscriptionConfig.condition(broadcasterId))
    );

    if (existingSubscription) {
      logger.log(
        "Twitch",
        `Subscription for ${subscriptionConfig.type} already exists on Twitch side, skipping creation`
      );
      ...
      if (!exists) {
        ...
        logger.log(
          "Twitch",
          `Updated local subscription records for ${subscriptionConfig.type}`
        );
      }
    ...
    try {
      ...
      if (error.response && error.response.status === 409) {
        logger.log(
          "Twitch",
          `409 Conflict creating subscription for ${subscriptionConfig.type}. Subscription likely exists.`
        );

        // Try to fetch again to get the current status
        const refreshedSubs = await fetchCurrentTwitchSubscriptions(userId);
        const matchingSub = refreshedSubs.find(
          (sub) =>
            sub.type === subscriptionConfig.type &&
            sub.version === subscriptionConfig.version
        );
        ...
        if (matchingSub) {
          ...
          const subscriptions =
            user.twitch_tokens?.streamer?.subscriptions || [];

          // Check if we already have this subscription
          const existingIndex = subscriptions.findIndex(
            (sub) => sub.id === matchingSub.id
          );
